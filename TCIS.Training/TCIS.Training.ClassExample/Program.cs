...
namespace TCIS.Training.ClassExample
{
    class Program
    {
        ...
        static void DummyData()
        {
            ...
            {
                var rnd = $"{baseSalaryTeacher[random.Next(baseSalaryTeacher.Count)]}";
                var ex = new SalaryClassification
                {
                    Id = i + 1,
                    Level = random.Next(1, 4),
                    BaseSalary = Convert.ToDouble(rnd)
                };
        ...
        static void Main(string[] args)
        {
            ...
            #region Tính Lương Giáo Viên
            foreach (var calculateSalary  in TeacherSalarys)
            {
                var teacherSalary = TeacherSalarys.Where(x => x.Teacher.TeachId == calculateSalary.Id);
